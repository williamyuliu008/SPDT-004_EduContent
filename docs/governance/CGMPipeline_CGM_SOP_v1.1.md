...
| 详细执行规范 | — | — | — | 见 `1_ingest/SPEC.md`（M0/M1/M2 子模块手册） |
...
|-----------------|----------|----------|
| `书法` | 书法 | `墨骨山河_`、`CHAIN_CAFA_Ep*` |
| `古诗文` | 古诗文 | `GAOKAO_`、`CHAIN_` |
| `历史` | 历史 | `CHAIN_`（不含GAOKAO/墨骨山河/CAFA前缀） |
| `地理` | 地理 | `GEO_山河_*`、`GEO_人地_*`、`GEO_区域_*` |
| `政治` | 政治 | `POLI_经济_*`、`POLI_政治_*`、`POLI_哲学_*`、`POLI_法律_*`、`POLI_时事_*` |

> **强制规则**：内容包导入时，若 chain_id 不含上述任何前缀，默认归入 `HISTORY`。
> 若内容专家有意将链放入其他学科，必须在 chain_meta JSON 中显式声明 `subject` 字段。

**地理科目标题映射规范**（来源：geo_shanhe_2026 经验，2026-08-01）：

App ImportService.ets 中维护 `getGeographyDisplayTitle()` 函数，chain_title 以 meta.json 中的 `chain_name` 为准。链前缀编号（`1_`~`16_`）用于 App 链列表排序：
...
**政治科目标题映射规范**（来源：上海高考思想政治考纲，2026-08-02）：

App ImportService.ets 中维护 `getPoliticsDisplayTitle()` 函数，chain_title 以 meta.json 中的 `chain_name` 为准。链前缀编号（`1_`~`16__`）用于 App 链列表排序：
...
哲学常识链（4条）：
  POLI_哲学_唯物辩证   → 9_唯物辩证法
  POLI_哲学_认识实践   → 10_认识与实践
  POLI_哲学_历史唯物   → 11_历史唯物主义
  POLI_哲学_唯物论     → 12_唯物论基础
...
| Q9 | App rawfile JSON 无编码安全规范 | **App 导入失败** | ✅ **v1.2已交付（JSON铁律+检查清单）** | ✅ Mavis |
| Q10 | 地理等新科目 App 集成无 SOP 指引 | 每次新科目均需摸索 | ✅ **v1.2已交付（地理映射+App规范）** | ✅ Mavis |
| Q11 | M2 Mock 模式 audit_log 无标记 | 质量报告失真 | 🟡 待优化 | 内容专家 |

---

## 九·附：App rawfile JSON 卡片包集成规范

> **来源**：地理山河系列（geo_shanhe_2026）App 接入经验（2026-08-01）
> **适用范围**：非视频类科目内容包（卡片包）导入入境 App

### JSON 文件编码铁律

> ⚠️ **JSON 文件必须全程 Python 读写，任何步骤不得经 PowerShell 管道！**
>
> **违规操作（禁止）**：
> ```powershell
> # 禁止：PowerShell 重定向会破坏 UTF-8 编码
> python script.py > output.json
> Get-Content raw.json | python process.py > fixed.json
> ```
>
> **正确操作**：
> ```python
...
> ```python
> # BOM 检测
> f = open('xxx.json', 'rb'); print(f.read(3) == b'\xef\xbb\xbf'); f.close()
>
> # JSON 有效性
> import json; json.load(open('xxx.json', 'r', encoding='utf-8'))
> ```
...
def check_json(path):
    ...
    with open(path, 'rb') as f:
        if f.read(3) == b'\xef\xbb\xbf':
            errors.append(f'BOM detected in {path}')
    ...
    # 3. 控制字符检测（\r 在字符串内容内）
    ctrl = [(i, hex(ord(c))) for i, c in enumerate(raw)
            if ord(c) < 32 and c not in '\n\t']
    if ctrl:
        errors.append(f'Control chars found: {ctrl[:3]}')

    # 4. 必需字段检查
    for field in ['package_id', 'package_title', 'node_cards', 'total_cards', 'total_chains']:
        if field not in obj:
...
if __name__ == '__main__':
    path = sys.argv[1] if len(sys.argv) > 1 else 'entry/src/main/resources/rawfile/geo_cards.json'
...
### App 集成完整检查清单（每次部署前必跑）

- [ ] **JSON 有效性**：`python check_rawfile_json.py <path>` 返回 OK
- [ ] **BOM 检测**：`python -c "f=open('xxx','rb'); print(f.read(3)==b'\xef\xbb\xbf'); f.close()"` → False
- [ ] **chain_id 一致性**：rawfile JSON 中的 chain_id 与 meta.json 完全一致（无前缀差异，如 `GEO_山河_珠江` ≠ `GEO_山河_珠江三角洲`）
- [ ] **ImportService 映射更新**：新增链时，在 `getGeographyDisplayTitle()` / `getCalligraphyDisplayTitle()` 等函数中添加标题映射
- [ ] **subject 推断覆盖**：在 `inferSubject()` 中添加新链的前缀判断（`startsWith` 链）
- [ ] **HVigor 增量构建**：无 ArkTS 编译错误
- [ ] **hdc 部署成功**：`hdc install` 返回 `install bundle successfully`
- [ ] **App 导入验证**：点击"一键导入"按钮 → HiLog 无 ERROR → 卡组学习页面正常显示
...
| v1.2 | 2026-08-02 | 新增附录D（hvigor build workspace 修复方案） | Mavis |

---

### D. hvigor Build Workspace 修复（2026-08-02）

**问题**：`C:\Users\willi\.mavis\workspace\` 目录中曾存在 `.hvigor/` + `build-profile.json5` 等 hvigor 配置文件。DevEco hvigor 在启动时会**向上扫描最近的 `.hvigor` 目录**作为项目根目录，因此即使在 D:\ 项目路径下执行 hvigor，它仍会错误地读取 workspace 目录的 `build-profile.json5`，导致 `00303149 Configuration Error: Path not found`。

**症状**：
- hvigor 报错：`00303149 Configuration Error` → `Path not found. At file: C:\Users\willi\.mavis\workspace\entry2`
- 错误提示：`Please check field: modules in file: C:\Users\willi\.mavis\workspace\build-profile.json5`
- `--no-daemon` 和 `--stop-daemon-all` 均无法解决（因为 daemon 早已不存在）

**根因**：hvigor 会从当前工作目录向上遍历父目录，寻找最近的 `.hvigor` 文件夹作为项目根。`.mavis` → `.minimax` 的 junction 导致路径解析混乱。

**修复方案**：workspace 中的 hvigor 相关文件已全部重命名为 `_DISABLED` 后缀，使 hvigor 只能使用 D:\ 的真实项目（勿删除或恢复这些 `_DISABLED` 目录）。
...
Push-Location 'D:\2_products\education\SPDT-004_EduContent\5_deliver\TextExperienceAPP\apps\rujing'
& $HVIGOR --mode module -p product=default assembleHap --no-daemon --analyze=false
Pop-Location
```

**hdc 部署**（设备连接时）：
```powershell
$HDC = 'C:/Users/willi/AppData/Local/OpenHarmony/Sdk/26.0.0/toolchains/hdc.exe'
$HAP = 'D:\2_products\education\SPDT-004_EduContent\...\entry\build\default\outputs\default\entry-default-signed.hap'
$DEV = '7GL0226313016118'  # 设备ID（每次可能不同，hdc list targets 确认）
& $HDC -t $DEV install "$HAP"
```
