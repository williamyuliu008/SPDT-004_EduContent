...
> **管线名称**：知识传递引擎
> **版本**：v1.1 | **创建日期**：2026-07-16 | **更新**：2026-07-29
...
```
1_ingest  知识摄入      →  高质量原料筛选 + 考纲对齐
2_structure 结构化加工   →  TextExperience: 微剧本 + Flashcard + 电子书 + 广播剧音频
3_render  渲染生产      →  SPDT-KTE: 管线编排 + PT-VFX 视觉渲染
4_adapt   自适应编排    →  AdaptivePrepPlatform: 薄弱点驱动 + 知识图谱
5_deliver 触达交付      →  PT-RUJ "入境" App: 链阅读 + Flashcard复习 + 视频播放
```
...
| PT | 名称 | 定位 | 物理位置（v2.3） |
|---|---|---|---|
| **TextExperience** | 内容工厂 | 微剧本 + Flashcard + 电子书 + 广播剧音频 | `SPDT-004/2_structure/TextExperience` |
| **PT-VFX** | VideoFactory | 视觉渲染引擎：Pillow 渲染帧 + MP4 视频合成 | `SPDT-004/3_render/video_factory` |
| **PT-RUJ** | Rujing（入境） | 移动交付终端：鸿蒙 App，链阅读 + 卡片复习 + 视频播放 | `SPDT-004/5_deliver/TextExperienceAPP/apps/rujing` |

> **2026-07-29 v2.3**：SPDT-KTE 从 SPDT-004 顶层降级为 `3_render/SPDT-KTE/` 子模块，PT 路径同步更新。
...
├── README.md                      ← 本文件
│
├── PT-038 @ [junction]            ← → ../../2_structure/TextExperience
├── PT-VFX  @ [junction]           ← → ../../3_render/video_factory
├── PT-RUJ  @ [junction]           ← → ../../5_deliver/TextExperienceAPP/apps/rujing
│
...
- [三项目协同方案（代码层）](docs/../PT-038/docs/三项目协同方案_代码层审计与补全路线图.md) — 实施路线图
- [TextExperience 内容工厂](PT-038/docs/TextExperience_SOP.md) — SOP、战略分析、配置包
