...
**通用备考操作系统**：平台层（知识图谱+5 Agent调度+自适应训练），编排消费 TextExperience 生产的课程内容包。

**物理位置**：`platform/AdaptivePrepPlatform/`

与 TextExperience 的关系：TextExperience（`platform/TextExperience/`）是**主力生产模块**，产出剧本杀/古史知识链；PT-037 负责**编排消费**（知识图谱 + 自适应训练调度）。
