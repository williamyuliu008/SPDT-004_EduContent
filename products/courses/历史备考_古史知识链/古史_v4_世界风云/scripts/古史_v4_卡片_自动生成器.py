...
古史_v4_世界风云  卡片自动生成器
============================================================
从古史_v4 6集 episode Python 数据文件中提取 knowledge_chains，
自动派生三类卡片：节点卡 + 心法卡 + 链卡。

输出：
  - 古史_v4_全18链卡片包.json
  - 古史_v4_全18链_Anki导入.csv
"""
...
def derive_node_cards(chain, ep):
    ...
    for node in chain.get('key_nodes', []):
        prefix = chain['chain_id'].split('_')[1]  # 世界近代/世界现代
        chain_num = chain['chain_id'].split('_')[-1]
        node_num = node.get('node_id', 'N01').split('_')[-1]
        card_id = f"H_W4_{prefix}_{chain_num}_{node_num}"
        ...
        cards.append({"card_id": card_id, "type": "node", "front": front, "back": back,
                       "chain_id": chain['chain_id'], "ep": ep.get('ep', '?'),
                       "source": ep.get('title', '')})
    return cards
...
def derive_strategy_cards(chain, ep):
    ...
    if ctype in ('原因类', '变革类', '制度类'):
        card_id = f"S_W4_{prefix}_{chain_num}_01"
        front = f"[{chain['chain_id']}] {chain['chain_title']}\n为什么发生？"
        back = chain.get('cause_summary', chain.get('strategy', '【请补充】'))
        if chain.get('memorization_tips'):
            back += f"\n\n【口诀】{chain['memorization_tips']}"
    ...
    if ctype in ('影响类', '变革类'):
        card_id = f"S_W4_{prefix}_{chain_num}_02"
        front = f"[{chain['chain_id']}] {chain['chain_title']}\n产生了什么后果？"
        back = chain.get('impact_summary', '【请补充】')
        cards.append({"card_id": card_id, "type": "strategy_impact", "front": front, "back": back,
                       "chain_id": chain['chain_id'], "ep": ep_num, "source": ep.get('title', '')})

    if ctype == '变革类':
        card_id = f"S_W4_{prefix}_{chain_num}_03"
        front = f"[{chain['chain_id']}] {chain['chain_title']}\n变革的关键转折点是什么？"
        back = chain.get('strategy', '【请补充】')
...
def derive_chain_card(chain, ep):
    prefix = chain['chain_id'].split('_')[1]
    chain_num = chain['chain_id'].split('_')[-1]
    card_id = f"L_W4_{prefix}_{chain_num}"
    ...
    return {"card_id": card_id, "type": "chain", "front": front, "back": back,
            "chain_id": chain['chain_id'], "ep": ep.get('ep','?'), "source": ep.get('title','')}
...
def main():
    ...
    json_path = os.path.join(cards_dir, "古史_v4_全18链卡片包.json")
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump({"version": "1.0", "series": "世界风云", "total_chains": chain_count,
    ...
    csv_path = os.path.join(cards_dir, "古史_v4_全18链_Anki导入.csv")
